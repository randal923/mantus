...
| **D** | movement-triggered — Step-in / step-out (or add-item) tiles; extends `PressurePlateRegistry` or a sibling movement registry. Unconditional step-in portals now have a data seam: `QUEST_TELEPORTS` in `server/src/action/questTeleportTables.ts` (77 rows after the 2026-08-11 teleport audit; `yarn playtest:quest-teleports` walks every one). Portals whose destination depends on the player have their own tick-time services beside it (`AdventurersGuildExitService`, `ElementalShrineService`). Storage/boss-gated portals still do NOT fit the table — ~370 map tiles wait on the quest-storage and boss-cooldown platforms (enumerated in `TODO.md`). | 45 |
