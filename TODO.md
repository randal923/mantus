...
- **World-action parity JSON carries hand edits the classifier does not
  ...
  flips `adventurers_stone.lua`, `temple_scroll.lua` and the quest-teleport
  entries back to `deferred`: they were marked implemented by editing
  `content/canary-world-action-parity.json` directly without a
  `FILE_DISPOSITIONS` entry in `tools/classifyWorldActionRegistration.mjs`
  (the gold converter got both). Fix: add the three dispositions and confirm
  regeneration is a no-op. Owner: content.
