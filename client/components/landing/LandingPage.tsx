...
import { LandingNewsArticle } from "./LandingNewsArticle";
  ...
  return (
    <PublicSiteShell>
      ...
      <main className="relative z-10 mx-auto grid w-full max-w-7xl gap-7 px-4 pt-8 pb-12 sm:px-6 md:grid-cols-[14.5rem_minmax(0,1fr)] xl:grid-cols-[14.5rem_minmax(0,1fr)_18rem]">
        ...
        <div
          id="latest-news"
          className="order-1 flex min-w-0 scroll-mt-28 flex-col gap-5 md:order-2"
        >
          ...
          {LANDING_NEWS_ARTICLES.map((article) => (
            <LandingNewsArticle key={article.id} article={article} />
          ))}
