...
- `move-map-item` allows throws to any existing tile within `THROW_RANGE` (7) with no line-of-sight or walkability check (Canary uses `canThrowObjectTo`; `World.hasLineOfSight` already exists for combat — reuse it). The same laxness exists in `drop-item`.
