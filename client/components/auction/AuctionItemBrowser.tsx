    ...
    >
      <header className="border-b border-ui-gold/15 px-3 py-3">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <span className="rounded-full border border-ui-stone-light/15 bg-black/35 px-2 py-0.5 text-xs tabular-nums text-ui-muted">
        ...
        {CATEGORIES.map((categoryOption) => {
          ...
          return (
            <button
              ...
              className={`min-w-max rounded-md border px-2.5 py-2 text-left text-xs font-semibold tracking-wider uppercase outline-none transition-[border-color,background-color,color] focus-visible:ring-2 focus-visible:ring-ui-gold/60 xl:min-w-0 ${
                active
      ...
      <div className="ui-scrollbar min-h-0 flex-1 overflow-y-auto p-2">
        {items.length === 0 ? (
          <div className="flex min-h-36 items-center justify-center rounded-lg border border-dashed border-ui-stone-light/20 bg-black/15 px-5 text-center text-sm leading-6 text-ui-muted">
            {t("auction.noItems")}
          </div>
        ) : (
                ...
                <li key={item.id}>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block truncate text-xs tracking-wide text-ui-muted">
                        {t(`auction.categories.${item.category}`)}
                      </span>
                    </span>
                    <span className="shrink-0 text-right">
                      <span className="block text-sm font-semibold tabular-nums text-ui-text">
