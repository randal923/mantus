...
/**
 * Unlock stage for one bestiary entry: 0 = never killed (silhouette only),
 * 1 = counter visible, 2 = combat stats revealed, 3 = resistances and
 * locations revealed, 4 = completed (full loot, charm points awarded).
 */
...
export const bestiaryCreatureEntrySchema = z
  .object({
    raceId: raceIdSchema,
    /** Catalog data; the client masks the name while stage is 0. */
...
/** One loot row; id/sprite/name stay hidden (0/absent) until the stage reveals them. */
export const bestiaryLootEntrySchema = z
  .object({
    itemTypeId: z.number().int().min(0).max(65_535),
    spriteId: z.number().int().min(0).max(4_000_000),
...
export const bestiaryMonsterStateMessageSchema = z
  .object({
    ...
    className: bestiaryClassSchema,
    outfit: creatureOutfitSchema,
    stage: bestiaryStageSchema.refine((stage) => stage >= 1),
    kills: killsSchema,
    ...
    resistances: z.array(bestiaryResistanceSchema).max(12).optional(),
    /** Present from stage 3. */
    locations: z
      .string()
      .max(BESTIARY_LIMITS.maxLocationsLength)
      .optional(),
  })
  .strict();
...
export const bosstiaryEntrySchema = z
  .object({
    raceId: raceIdSchema,
    /** Catalog data; the client masks the name while kills is 0. */
    name: z.string().min(1).max(BESTIARY_LIMITS.maxNameLength),
    outfit: creatureOutfitSchema,
