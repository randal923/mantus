...
export function WikiItemDetails({
  ...
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/75 p-3 backdrop-blur-xs">
      <section
        aria-label={item.name}
        className="ui-panel-frame ui-scrollbar relative isolate max-h-full w-full max-w-3xl overflow-y-auto p-5 shadow-2xl sm:p-7"
      >
        <div
          aria-hidden
        ...
        <header className="flex items-start gap-4">
          <span className="flex size-24 shrink-0 items-center justify-center rounded-md border border-ui-gold/25 bg-black/35 shadow-inner">
        ...
        <div aria-hidden className="ui-divider my-5" />

        {item.description && (
          <p className="text-sm leading-6 text-ui-text/85">{item.description}</p>
        )}
